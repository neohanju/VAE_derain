...
function util.loadImagePairs(imageIndex, synthsizedIndex)
...
function util.imageCatH(image1, image2)
	return torch.cat(image1, image2, 3)
end

function util.imageCatV(image1, image2)
...
-- normalize a set of images, called batch
...
-- return image fulled with L (3 channels)
...
-- from a image (3-dim) to a set of image (4-dim) contaning a single image
function util.toTrivialBatch(input)
	return input:reshape(1,input:size(1),input:size(2),input:size(3))
end

-- return a fist image from the batch (4-dim)
function util.fromTrivialBatch(input)
    return input[1]
end

-- rescale image into a squre form and handle a single channel image
...
-- get image aspect ratio with file path
